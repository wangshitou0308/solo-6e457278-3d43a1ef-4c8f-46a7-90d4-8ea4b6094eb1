...
app = FastAPI(
    ...
    description=(
        ...
        "总字节数、整体 SHA-256、脱敏策略与**必填的 token_context 关联域**——会话"
        "不提供全局域回退，可带 Idempotency-Key），再用 "
        "`Content-Range: bytes <start>-<end>/<total>` 乱序提交分片；分片以 0600 "
        "权限流式暂存并逐片登记 SHA-256 索引，进度接口返回已收/缺失区间。相同区间内容"
        "一致的重传幂等回放（200）；区间重叠冲突、越界或逐片摘要不符一律拒绝且保留已有"
        "分片。会话与分片索引在服务重启后恢复；除访问时惰性过期外，后台清扫线程按 "
        "`REDACTOR_UPLOAD_SWEEP_SECONDS`（默认 60 秒）周期主动清理到期会话的暂存文件。"
        "完成时要求分片恰好覆盖全部字节并通过整体 SHA-256 校验：NDJSON 转入既有流式"
        "作业，ZIP 经原有安全检查后转入诊断包作业，策略、令牌关联域与 Idempotency-Key "
        "全部沿用创建时的声明；完成后会话禁止继续写入。\n\n"
        "## 脱敏结果完整性凭证（receipt）\n"
...
@app.post(
    ...
    description=(
        ...
        "**必填** `token_context`（最长 128 字符；断点续传会话必须声明令牌关联域，"
        "不提供全局域回退，服务只持久化不可逆域标识）；随后用 "
        "`PUT .../chunks` 按 `Content-Range` 乱序提交分片，全部覆盖后调用 "
        "`.../complete` 转入对应的脱敏作业。\n\n"
        "会话默认 24 小时过期（`expires_in_seconds` 可调，最长 7 天）；除访问时"
        "惰性过期外，后台清扫线程按 `REDACTOR_UPLOAD_SWEEP_SECONDS`（默认 60 秒）"
        "周期主动清理到期会话的暂存文件。携带 `Idempotency-Key` 时：相同键且"
        "**类型、总字节、整体摘要、规范化策略与关联域**五者一致才回放既有会话"
        "（200），任一不同返回 **409**；该键在完成时继续绑定转入的流式/诊断包作业。"
    ),
    status_code=201,
    responses={
